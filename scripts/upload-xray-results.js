...
const XRAY_BASE_URL = process.env.XRAY_API_BASE_URL || process.env.XRAY_BASE_URL || 'https://xray.cloud.xpand-it.com';
    ...
    const req = protocol.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          const executionKeyFromHeader = res.headers['x-ray-test-execution-key'] || res.headers['x-test-execution-key'];

          try {
            const response = data ? JSON.parse(data) : {};

            // Extract and display test execution key from various sources
            let executionKey = response.testExecutionKey || response.key || response.id || executionKeyFromHeader;
            ...
            if (executionKey) {
              ...
              console.log('💡 You may need to check your Xray project to find the newly created Test Execution');
            }
            ...
            const executionInfo = {
              ...
              url: executionKey ? `${JIRA_BASE_URL || 'https://your-jira-instance.atlassian.net'}/browse/${executionKey}` : null,
            ...
            const executionInfo = {
              ...
              url: executionKeyFromHeader ? `${JIRA_BASE_URL || 'https://your-jira-instance.atlassian.net'}/browse/${executionKeyFromHeader}` : null,
              testsUploaded: payload.tests ? payload.tests.length : 0,
              rawResponse: data,
              headers: res.headers
            };
