...
describe("Where Is My Train - Test Suite", function () {
  ...
  it("TC01: Verify Home Page is displayed", async function () {
  ...
  it("TC02: Search trains From New Delhi To Mumbai CSMT and verify results", async function () {
  ...
  it("TC03: Search train by number 22308 and verify results on SearchByTrainNoTrainName page", async function () {
  ...
  it("TC04: Search trains by station New Delhi and verify results on SearchResultsForTrains page", async function () {
    await trainHelper.searchByStation("New Delhi", "NDLS");
    const trainNumbers = await trainHelper.getStationSearchResults();
    expect(trainNumbers.length).to.be.greaterThan(0);
  });

  it("TC05: Select a train from SearchResultsForTrains and verify TrainStatusView page", async function () {
    await trainHelper.searchByStation("New Delhi", "NDLS");
  ...
  it("TC06: Verify recent search appears on the Home screen after searching by train number", async function () {
    await trainHelper.searchByTrainNumber("22308");
